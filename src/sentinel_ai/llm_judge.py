...
class LLMJudge:
    ...
    _SCORE_PATTERNS: list[re.Pattern[str]] = [
        ...
        re.compile(r"\b([0-4](?:\.[0-9]+)?)\s*/\s*4\b"),
    ...
    def _parse_score(self, text: str) -> JudgeResult:
        ...
        try:
            ...
            if isinstance(data, dict) and "score" in data:
                return JudgeResult(
                    score=float(data["score"]), raw_response=text, metadata=data
                )
        except (json.JSONDecodeError, ValueError, TypeError):
            pass

        # Fallback: regex chain
        for pattern in self._SCORE_PATTERNS:
            match = pattern.search(text)
            if match:
                try:
                    return JudgeResult(score=float(match.group(1)), raw_response=text)
                except ValueError:
                    continue
